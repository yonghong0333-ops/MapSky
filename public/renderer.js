    ...
    if (pushForm && !pushForm.dataset.bound) {
      ...
      pushForm.addEventListener("submit", async (e) => {
        ...
        try {
          ...
          if (pushMsg) {
            const webPart = `網頁 ${result.sent} / ${result.total}${result.expired ? `（清掉 ${result.expired} 個失效訂閱）` : ""}`;
            const iosPart = result.totalIos > 0 || result.sentIos > 0
              ? `，iOS ${result.sentIos} / ${result.totalIos}${result.expiredIos ? `（清掉 ${result.expiredIos} 個失效裝置）` : ""}`
              : "";
            pushMsg.textContent = `已發送給 ${webPart}${iosPart}`;
          }
          pushForm.reset();
        } catch (e) {
...
// 動態島原生端「天氣」模式用的圖示，走跟 iconForWx()／WX_ICON_FILES 同一套
// camelCase key（sunny／partlyCloudy／...），不是上面給 SF Symbol 用的
...
async function sendToDynamicIsland() {
  ...
  if (adventureSelectedMode === "uv") {
    ...
    await window.MapSkyNative.updateDynamicIsland({
      mode: "uv",
      cityName,
      uvValue: `${uv.uvIndex}（${uv.level}）`,
      durationMinutes: adventureDurationMinutes,
    });
    return;
  }
  ...
  if (adventureSelectedMode === "wind") {
    ...
    await window.MapSkyNative.updateDynamicIsland({
      mode: "wind",
      cityName,
      windValue: `${wind.windSpeed} m/s`,
      durationMinutes: adventureDurationMinutes,
    });
    return;
  }

  if (adventureSelectedMode === "typhoon") {
    const primary = tyWarnings && tyWarnings[0];
    if (!primary) return; // 沒有生效中的颱風警報就不送，避免顯示空白內容
    await window.MapSkyNative.updateDynamicIsland({
      mode: "typhoon",
      cityName,
      typhoonText: `${primary.intensity || ""}颱風 ${primary.name}`,
      durationMinutes: adventureDurationMinutes,
    });
    return;
  }
