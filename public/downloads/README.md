...
網站的「下載 Windows 版」按鈕直接連到這裡的 `MapSky-Setup.exe`，網址是：

    https://mapskyapp.vercel.app/downloads/MapSky-Setup.exe
...
3. 把你包好的安裝檔拖進去，**檔名務必存成 `MapSky-Setup.exe`**（跟現有的同名，
   上傳時會直接取代舊檔案）
...
檔名一定要是 `MapSky-Setup.exe`，網站按鈕是寫死抓這個檔名，改了別的名字
按鈕會抓不到。
